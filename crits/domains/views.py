...
def tld_update(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logger.error(f"TLD Upload: File size: {filedata.size}, Name: {filedata.name}")
            ...
            if result['success']:
                ...
                error_msg = result.get('message', 'Unknown error occurred during TLD upload')
                response = {'success': False, 'message': error_msg, 'form': form.as_table()}
        else:
            # Debug form errors
            import logging
            logger = logging.getLogger(__name__)
            logger.error(f"TLD Upload form errors: {form.errors}")
            response = {'success': False, 'message': f'Form validation failed: {form.errors}', 'form': form.as_table()}
