...
def handle_eml(data, sourcename, reference, method, tlp, user, campaign=None,
    ...
    if start >= 0 and end >= 0:
        ...
        for preheaders in premail.splitlines():
            ...
            if mfpos > -1:
                try:
                    mailfrom = six.text_type(preheaders[mfpos + 10:])
                except UnicodeDecodeError:
                    mailfrom = six.text_type(preheaders[mfpos + 10:], errors="replace")
            rcpos = preheaders.find("RCPT TO")
            if rcpos > -1:
                try:
                    rcptto = six.text_type(preheaders[rcpos + 9:])
                except UnicodeDecodeError:
                    rcptto = six.text_type(preheaders[rcpos + 9:], errors="replace")
        if mailfrom:
    ...
    for part in msg.walk():
        ...
        if part.get_content_maintype() == "text":
            ...
            if content:
                try:
                    message_part = six.text_type(content)
                except UnicodeDecodeError:
                    message_part = six.text_type(content, errors="replace")
        ...
        if attach is not None and len(attach):
            ...
            if filename is not None:
                try:
                    ...
                    filename = six.text_type(filename, errors="replace")
            else:
                filename = md5
...
def dict_to_email(d, save_unsupported=True):
    ...
    if 'to' in d and isinstance(d['to'], six.string_types) and len(d['to']) > 0:
        d['to'] = [d['to']]

    if 'cc' in d and isinstance(d['cc'], six.string_types) and len(d['cc']) > 0:
        d['cc'] = [d['cc']]
