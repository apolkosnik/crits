...
def is_ajax(request):
    """Django 4.2+ compatible AJAX detection"""
...
django_version = tuple(map(int, get_version().split('.')[:2]))
...
def login(request):
    ...
    if (request.user.is_authenticated if django_version >= (1, 10) else request.user.is_authenticated()) and user.has_access_to(GeneralACL.WEB_INTERFACE) and not is_ajax(request):
    ...
    if request.method == 'POST' and is_ajax(request):
        ...
        if user:
            ...
            if 'organization' not in user:
                user['organization'] = settings.COMPANY_NAME
        else:
            user = {'username': '',
                    'roles': '',
...
def add_update_ticket(request, method, type_=None, id_=None):
    ...
    if method =="remove" and request.method == "POST" and is_ajax(request):
        date = datetime.datetime.strptime(request.POST['key'],
                                            settings.PY_DATETIME_FORMAT)
